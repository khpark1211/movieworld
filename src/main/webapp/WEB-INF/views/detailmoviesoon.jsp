				...
				<tbody>
					<tr class="view_tr">
						...
						<td class="view_td"><c:choose>
								<c:when test="${empty movievo2.file_name}">
									<b> 첨부파일 없음 </b>
								</c:when>
								<c:otherwise>
									<img src="<c:url value='resources/upload/mvposter/${movievo2.file_name}'/>"
										width="150px">
								</c:otherwise>
							</c:choose></td>
					</tr>
					<tr class="view_tr">
						...
						<td class="view_td">${movievo2.title}</td>
					</tr>
					<tr class="view_tr">
						<th class="view_th">평점</th>
						<td class="view_td">${movievo2.rate}</td>
					</tr>
					<tr class="view_tr">
						<th class="view_th">상영시간</th>
						<td class="view_td">${movievo2.running_time}</td>
					</tr>

					<tr class="view_tr">
						<th class="view_th">개봉일</th>
						<td class="view_td">${movievo2.open_Date}</td>
					</tr>
					
					<tr class="view_tr">
						<th class="view_th">줄거리</th>
						<td style="height:250px; text-align: left; padding-left: 30px;" class="view_td"><pre>${movievo2.story}</pre></td>
					</tr>
					...
					<c:choose>
						<c:when test="${mvo.mem_name == '운영지기'}">
							<tr>
								...
								<input type="hidden" name="idx" value="${movievo2.idx}">
							</tr>
						</c:when>
						<c:otherwise>
							<tr>
